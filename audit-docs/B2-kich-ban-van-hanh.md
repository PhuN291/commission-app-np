...
Status: WIP B2.1 (5 kịch bản persona). Group A (VĐ-1 đến VĐ-10) đã chốt. Sang B2.2 BPMN sau khi đóng VĐ-4 sub-questions.
Ngày: 2026-04-25 (update lần 2)
Phụ thuộc: B1 FINAL update lần 2 (xem `B1-bien-ban-context.md`), ADR-001 (Shift architecture J), PENDING-ITEMS.md (blocker và assumption)
...
4. Lễ tân tiếp đón, check iHOS xem khách cũ hay mới
...
App HH:
- Sinh đơn ở iHOS, app nhận qua webhook
...
|---|---|---|---|---|---|---|
| P1 | ĐD-Sale | 4 | Điều dưỡng (phụ thủ thuật) | Sale (gọi lead, tạo đơn) | **Sale** (1 role duy nhất, gộp công ĐD vào Sale) | Mobile |
...
- %HH: Sale 3% [GIẢ ĐỊNH] (1 role duy nhất, đã gộp công ĐD vào Sale - chốt 01/05/2026)
...
| 9h00 | Khách A đến (lead Lan đã chốt hôm qua) | Lễ tân check iHOS, ra hồ sơ. Lan vào phòng tư vấn | Khách cũ, đã có mã |
| 9h10 | Tư vấn, khách A đồng ý gói khám tổng quát + xét nghiệm máu | Screen Tạo đơn (qua iHOS web hay app HH? GIẢ ĐỊNH: Lan tạo trên iHOS, app HH nhận webhook) | Đơn O-001 sinh ra |
...
| 9h20 | Khách thanh toán cho lễ tân | Không touch app HH | Lễ tân ấn paid trong iHOS |
...
| 9h50 | Khách thanh toán bổ sung phần siêu âm | Lễ tân ấn paid bổ sung trong iHOS | |
...
| 15h30 | Khách E đến, đồng ý làm thủ thuật phụ khoa luôn | Lan vừa là người tạo đơn (Sale) vừa phụ thủ thuật | OrderRoleAssignment chỉ 1 row Sale=Lan. HH Lan = 3% × net_profit (chốt 01/05: ĐD-Sale là 1 role gộp, không tách ĐD ra row riêng) |
...
- Q-2.1.C: Tạo đơn ở đâu (iHOS web, app HH, cả 2)?
...
| Giờ | Hoạt động | Touchpoint app | Ghi chú |
|---|---|---|---|
| 8h30 | Đến clinic, mở iHOS xem lịch khám hôm nay | Không touch app HH | iHOS là tool công việc chính |
...
| 9h30 | Khám khách A của Lan, chỉ định siêu âm | Webhook iHOS -> app HH gán {O-001, role=BS, user=BS Minh}. BS Minh không touch app | App tự động hoá |
| 12h00-13h30 | Nghỉ trưa | | |
...
### 2.4 Kịch bản P4: Kế toán Diễm

**Background**:
- Tên Diễm, 35 tuổi, kế toán full-time NP 2 năm
...
| 9h00 | Đối soát doanh thu hôm qua với iHOS | Mở web iHOS (không phải app HH) | |
...
| 11h00 | CEO gọi điện, "đơn O-099 anh adjust giảm HH Lan 200k vì lỗi tư vấn", "đơn O-105 anh cộng thêm HH Hằng 100k vì làm thủ thuật khó" | Diễm ghi tay vào Excel | [GIẢ ĐỊNH] Adjustment chỉ ghi tay, không có audit trail trong app |
...
- Đối soát iHOS (doanh thu) vs app HH (HH): hai source khác nhau, sai lệch khó truy
...
| 9h00 | Review từng nhân viên, quyết adjustment | | CEO duyệt từng đơn flag, nói tay với Diễm |
| 10h00 | Export file gửi Diễm | Screen Export, file Excel/PDF | |
| 11h00 | Diễm hỏi điều chỉnh, CEO nói tay | | Adjustment ghi Excel ngoài, không trong app |
| 16h00 | Diễm trình bảng lương, CEO duyệt | | Qua Zalo/email, không qua app |
...
- Không có HH, lương cứng
- Tool chính: iHOS (web/desktop ở quầy), điện thoại bàn, máy in hoá đơn
- Không dùng app HH

Vai trò trong workflow:
- 8h00: mở quầy, kiểm tra lịch hẹn iHOS
- Mỗi khách đến: chào, hỏi đã có hẹn chưa, search tên trong iHOS, dẫn vào phòng tư vấn (nếu chưa có hồ sơ thì tạo mới sau khi ĐD-Sale tư vấn xong)
- Sau khi đơn được tạo và confirm: thu tiền, in hoá đơn
- Cập nhật trạng thái paid trong iHOS (action này trigger webhook tới app HH)
...
| Q-2.1.C | Tạo đơn ở đâu (iHOS web, app HH, cả 2)? | Webhook contract |
...
### Vấn đề lớn (Group A) - Trạng thái sau audit 25/04/2026

| ID | Tóm tắt | Status | Chốt |
|---|---|---|---|
| VĐ-1 | Per-order vs per-item HH, công bằng giữa 2 ĐD-Sale cùng đơn | ĐÓNG | Per-order, cap 10% (giảm từ 15%). Chấp nhận micro-unfairness để giữ logic đơn giản. |
| VĐ-2 | ĐD-Sale tham gia tư vấn cùng người tạo đơn có ăn HH không | ĐÓNG | Không có case này tại NP. Người tạo đơn = người ăn HH role Sale. |
| VĐ-3 | TC architecture: lỡ Hà nghỉ, hoặc mai sau scale multi-shift | ĐÓNG | Phương án J (F + extension hooks). Schema additive: `priority` + `condition` JSON. Day-1 đơn giản, future-ready. Xem ADR-001. |
| VĐ-4 | Adjustment audit trail | ĐÓNG | Workflow đơn nguồn: KT trưởng (Diễm) tạo, CEO duyệt. Edit window 30 ngày + clawback kì sau. Transparent hiển thị NV. Auto Reward rule "thưởng target tháng" trong MVP. Phần phạt defer phase riêng (B-12). |
| VĐ-5 | App có nên enforce cap không | ĐÓNG | App KHÔNG enforce. CEO tự tính ngoài Excel. App chỉ hiển thị warning visual. |
| VĐ-6/7 | Refund toàn phần / 1 phần / sau payday | ĐÓNG | Cơ chế manual adjustment đã cover. Refund sau payday là business policy của NP, không lq dev. |
| VĐ-8 | Voucher cost ai chịu | ĐÓNG | Option A: NP chịu cost as marketing expense. HH tính trên giá niêm yết. |
| VĐ-9 | BH commission base | ĐÓNG | Option B Out-of-Pocket Only. HH chỉ tính trên phần khách trả ra túi. |
| VĐ-10 | Phát sinh dịch vụ trong đơn | ĐÓNG (scope shift) | Thuộc scope iHOS, App HH chỉ consume webhook payload khi đơn complete. Block bởi iHOS webhook contract, xem PENDING-ITEMS.md I-1. |

### Sub-questions VĐ-4 (đã chốt 25/04/2026)

| ID | Câu hỏi | Chốt |
|---|---|---|
| SQ-1 | Edit window adjustment sau payday | **30 ngày** sau payday |
| SQ-2 | Limit amount per adjustment | **Không limit amount**, mọi adjustment đều phải qua workflow tạo + duyệt |
| SQ-3 thưởng | Auto rule thưởng | **OK**: thưởng đạt target tháng (% bonus theo % overshoot) |
| SQ-3 phạt | Auto rule phạt | **Defer phase riêng** để discuss kỹ (xem B-12) |
| SQ-4 | Sale thấy lý do adjustment không | **Có**, transparent (giảm dispute) |
| Q-VĐ4 | KT trưởng | **Diễm** (1 người làm cả KT thường + KT trưởng, Case A) |
| C-1 | Workflow create + approve | KT trưởng tạo → CEO duyệt. Auto rule fire → vào queue CEO duyệt batch. No self-approval. |
| C-2 | Behavior khi sửa adjustment đã apply lương | Option 1: Clawback kì sau. Adjustment cũ giữ history, tạo CommissionRecord delta ở kì lương kế. |
...
| GĐ-2.8 | Tạo đơn nguồn ở iHOS, app HH chỉ nhận | Cần verify với iHOS API doc (RISK #1 B1) |
...
Trạng thái Group A: 9/10 VĐ đóng. Còn VĐ-4 với 5 sub-questions chờ anh chốt.

Sau khi anh chốt VĐ-4 sub-questions trong session tiếp theo:
- B2.1 chuyển FINAL
...
- Sang B2.4: dựng kịch bản kì lương ngày 5 chi tiết (kế toán + KT trưởng + CEO workflow)

Câu hỏi nhỏ (Q-2.x.x) có thể trả lời gọn theo đúng số ID cho dễ tra. Có thể defer sang B3 nếu không liên quan blocker.

## 6. Lịch sử update

| Date | Update |
|---|---|
| 2026-04-25 v1 | Tạo file. WIP B2.1 với 5 persona timeline + 4 VĐ lớn |
| 2026-04-25 v2 | Audit Group A (VĐ-1 đến VĐ-10): chốt 9/10 VĐ, đổi cap 15% → 10%, BH Option B, Voucher Option A, Shift architecture J, VĐ-10 shift về scope iHOS. Còn VĐ-4 sub-questions chờ chốt. |
| 2026-04-25 v3 | Đóng VĐ-4: KT trưởng tạo + CEO duyệt, edit 30 ngày + clawback, transparent NV, Auto Reward rule. Group A 10/10 ĐÓNG. Còn lại defer: phần phạt (B-12), iHOS contract (I-1 đến I-4), G1/G2/G5/G6 (theo timeline). Sang B2.2 BPMN. |
| 2026-05-01 v4 | Chốt ĐD-Sale = 1 role duy nhất (gộp công ĐD vào Sale). Sửa P1 persona table và Lan timeline 15h30. |
